...
export class PlaylistService {
    ...
    try {
      ...
      if (role !== checkedRole)
        throw new UnauthorizedException(
          'Permission denied, not logged in as an authorized user',
        );
      if (!user) {
  ...
  async findOne(req: Request, id: string) {
    ...
    if (listener.playlists.indexOf(id) === -1) {
      throw new UnauthorizedException(
        "The current listener is not the owner of the playlist or the playlist doesn't exist.",
      );
    }
  ...
  async addSong(req: Request, id: string) {
    ...
    const playlist = await this.playlistModel.findById(id);
    const { name, filePath, album, index } = req.body;
    const song = { name, filePath, album, index };
